...
#     Returns tuple (local_path, supabase_result_dict_or_None)
#     """
#     try:
...
#         try:
#             # attempt to upload to supabase under a session-specific path to avoid collisions
...
#         # If supabase upload succeeded, prefer storing the public_url (if present) in DB
#         db_file_path = None
...
#             # store path relative to USER_DATA_BASE_FOLDER where appropriate
#             try:
#                 rel = os.path.relpath(local_path, USER_DATA_BASE_FOLDER)
#             except Exception:
#                 rel = local_path
#             db_file_path = rel

#         # Save metadata to DB (if configured)
#         try:
...
def sanitize_transcript(text):
    """Clean Whisper/OpenAI transcripts from common TTS/SSML artifacts and stray tokens.

    This removes tags or artifact phrases (e.g., 'pitch 106', 'prosody', 'break time')
    often introduced when playback / SSML is captured in the recording.
    """
    if not text:
        return text

    cleaned = text
    # Remove SSML-like tags and common artifact words
    cleaned = re.sub(r"<[^>]+>", " ", cleaned)
    cleaned = re.sub(r"(?i)\b(pitch|prosody|break time|speak)\b\s*[:=]?\s*\d+%?", " ", cleaned)
    cleaned = re.sub(r"(?i)\b(pitch)\b\s*\d+", " ", cleaned)
    # Remove repeated non-alphanumeric sequences and collapse whitespace
...
def speech_to_text(audio_file_path):
    ...
    try:
        ...
        # Prefer explicit English for transcription to avoid wrong-language outputs
        with open(audio_file_path, "rb") as audio_file:
            transcript = openai.Audio.transcribe(
                model="whisper-1",
                file=audio_file,
        ...
        try:
            model = get_whisper_model()
            if model:
                # Try forcing English language on local transcription when supported
                try:
                    result = model.transcribe(audio_file_path, language='en')
                except TypeError:
                    result = model.transcribe(audio_file_path)

                # result can be dict-like or object-like depending on the wrapper
...
def submit_message():
    ...
    if audio_file and getattr(audio_file, 'filename', None):
        ...
        if os.path.exists(audio_path):
            ...
            try:
                audio = AudioSegment.from_file(audio_path)
                # Normalize to 16kHz, mono, 16-bit PCM which works well with Whisper
                try:
                    audio = audio.set_frame_rate(16000).set_channels(1).set_sample_width(2)
                except Exception:
                    pass
                audio.export(audio_path, format="wav")
                print("Audio converted/resampled to WAV (16kHz mono PCM) for transcription")
            except Exception as e:
                print(f"Error converting audio: {str(e)}")
                
            user_message = speech_to_text(audio_path)
            # Ensure any stray SSML/TTS tokens are removed from transcribed text
            user_message = sanitize_transcript(user_message)
        else:
            print(f"Failed to save audio file at: {audio_path}")
            return jsonify({'error': 'Failed to save audio file'}), 500
    ...
    try:
        ...
        if 'audio' in request.files:
            ...
            if audio_file:
                ...
                try:
                    with open(audio_path, 'rb') as f:
                        # Force English to avoid wrong-language transcriptions
                        resp = openai.Audio.transcribe(model='whisper-1', file=f, language='en')
                        user_transcript = resp.get('text') if isinstance(resp, dict) else ''
                        user_transcript = sanitize_transcript(user_transcript)
                except Exception:
                    user_transcript = speech_to_text(audio_path)
